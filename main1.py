...
class window:
    ...
    def __init__(self, root, title) -> None:
        ...
        self.user_input = ttk.Entry(self.root, width=40)
        self.user_input.grid(row=3, column=0, columnspan=5, padx=5, pady=10)

        # Create a button to submit user input
        submit_btn = ttk.Button(self.root, text='Submit Input', padding=5, width=15, command=self.process_user_input)
        submit_btn.grid(column=5, row=3, padx=5, pady=10)
    ...
    def process_user_input(self):
        ...
        try:
                # Parse the user input as a list of numbers
            input_numbers = [int(num) for num in input_text.split()]
            if len(input_numbers) != self.N:
                messagebox.showerror("Input Error", "Input must have the same length as the current array size.")
            else:
                    # Update the data with user input
                self.data = input_numbers
                self.display(self.N, self.data, self.colours)
        except ValueError:
                messagebox.showerror("Input Error", "Invalid input. Please enter a list of numbers.")
    ...
    # ----button selection of sorting algorithms----
